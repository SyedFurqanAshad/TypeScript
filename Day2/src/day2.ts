...
// const array = [1, 2, 3];
function subArrayCount(array: any[]): number {
  let count: number = 0;
...
const capitaliseNames = (names: string[]): string[] => {
...
const removeDuplicates = (arr: (number | string)[]): (number | string)[] => {
  const withoutDuplicates: (number | string)[] = [];
  arr.map(dup => {
...
const objToArray = (obj: objectType): any[][] => {
...
const unionTwoArrays = (arr1: number[], arr2: number[]): number[] => {
  const union: number[] = [];
